...
cfg_file = osp.join(osp.dirname(__file__), 'SfMLearner_config.yaml')
...
def get_specific_files(name_dir, name_ext, with_dir=False):
    ...
    if not osp.exists(name_dir):
        logging.error('path not exist: {}'.format(name_dir))
        ColorPrint.print_error('path not exist: {}'.format(name_dir))
        raise ValueError
...
class ColorPrint:
    ...
    def print_error(text):
        color_col = ColorPrint.create_color()
        print('{}{}{}'.format(color_col['CREDBG'], text, color_col['CEND']))
